...
// Title:            Programming Assignment 1: Whatsapp
...
public class WhatsApp
{
	 ...
	 *            prepopulation file. Print the USAGE message if exactly one
	 *            command line argument is not supplied. Print
	 *            FILE_DOES_NOT_EXIST if the file path given by the user on the
	 *            command line does not exist. Prints EXITING and exits when the
	...
	public static void main(String[] args)
	{
		if (args.length != 1)
		{
			Config.getInstance().getConsoleOutput().printf(Config.USAGE);
			System.exit(1);
		}
		...
		{
			Config.getInstance().getConsoleOutput().printf(Config.ERROR_IO);
			System.exit(1);
		...
		while (!command.equals("exit:"))
		{
			...
			while (Config.getInstance().getCurrentUser() != null)
			{
				...
				command = Config.getInstance().getConsoleInput().nextLine();
