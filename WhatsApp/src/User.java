...
public class User
{
	...
	public void removeBroadcastList(String broadcastNickname)
			throws WhatsAppException
	{
		if (!broadcastLists.contains(Helper.getBroadcastListFromNickname(
		...
		broadcastLists.remove(Helper.getBroadcastListFromNickname(
				Config.getInstance().getCurrentUser().getBroadcastLists(),
				broadcastNickname));
	}
	...
	{
		return friends.contains(Helper.getUserFromNickname(
				Config.getInstance().getCurrentUser().getFriends(), nickname));
	}
	...
	{
		for (BroadcastList b : broadcastLists)
		{
	...
	{
		if (friends.contains(Helper.getUserFromNickname(
				Config.getInstance().getCurrentUser().getFriends(), nickname))
				|| broadcastLists
						.contains(
			...
			String broadcastNickname)
	{
		...
		while (itr.hasNext())
		{
			BroadcastList b = itr.next();
			if (b.getNickname().equals(broadcastNickname)
					&& b.getMembers().contains(nickname))
			{
				return true;
			}
		}
	...
	public void addFriend(String nickname) throws WhatsAppException
	{
		...
		else if (friends.contains(Helper.getUserFromNickname(
		...
		{
			Config.getInstance().getConsoleOutput().printf(Config.CANT_LOCATE,
					nickname);
